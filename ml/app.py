...
def _manuals() -> list[dict[str, str | int]]:
    ...
    manuals, _ = state()
    return [
        {"slug": m.slug, "title": m.title, "sections": len(m.sections) - 1} for m in manuals.values()
    ]
...
def _section_view(manual: Manual, section: Section) -> dict[str, object]:
    """Раздел мануала в виде, удобном шаблону (заголовок, текст, дети)."""
    return {
        "id": section.id,
...
def _manual_toc(manual: Manual) -> list[dict[str, object]]:
    """Верхний уровень оглавления мануала (корень + его прямые подразделы)."""
    root = manual.sections.get(SECTION_ROOT)
    ids = [*((root.children if root else ()) or ()), SECTION_ROOT]
    return [_section_view(manual, manual.sections[i]) for i in ids if i in manual.sections]
...
# id assistant-сообщений, по которым прямо сейчас идёт стрим (нужно шаблонам)
...
async def knowledge_base_manual(request: Request, slug: str) -> HTMLResponse:
    """Страница мануала: ленивое дерево разделов слева, текст раздела справа."""
    manual = _manual(slug)
    if manual is None:
    ...
    return _render(
        ...
        manual=manual,
        toc=_manual_toc(manual),
        tab="knowledge",
    )


@app.get("/knowledge-base/{slug}/sections/{section_id}", response_class=HTMLResponse, **{
    "include_in_schema": False
})
async def knowledge_base_section(request: Request, slug: str, section_id: str) -> HTMLResponse:
    """HTMX-фрагменты: подгрузка секции и её подразделов при раскрытии дерева."""
    manual = _manual(slug)
    if manual is None:
        raise HTTPException(status_code=404, detail="Мануал не найден")
    ...
    return templates.TemplateResponse(
        request,
        "manual_subtree.html",
        {"manual": manual, "section": _section_view(manual, section)},
    )


@app.get("/chats", response_class=HTMLResponse, include_in_schema=False)
async def list_chats(request: Request) -> HTMLResponse:
    return _render(request, "chat_list.html")
...
async def create_chat(
    ...
    chat = chat_service.create_chat(system_prompt)
    _set_active(chat.id)
    response = _render(request, "create_result.html", chat=chat)
...
async def get_chat(request: Request, chat_id: str) -> HTMLResponse:
    ...
    _set_active(chat.id)
    view = ChatView(chat, streaming_id=_streaming_message_id(chat.id))
    if request.headers.get("HX-Request") == "true":
        return _render(request, "chat_panel.html", chat=view)
    return _render(request, "index.html", chat=view)
...
async def delete_chat(request: Request, chat_id: str) -> HTMLResponse:
    ...
    return _render(request, "delete_result.html", chat=_panel_chat())
...
async def send_message(request: Request, chat_id: str, message: str = Form(...)) -> HTMLResponse:
    ...
    _set_active(chat_id)
    user_msg, assistant_msg, updated = chat_service.prepare_turn(chat_id, text)
    _streaming.add(assistant_msg.id)
    view = ChatView(updated, streaming_id=assistant_msg.id)
    return _render(
        request,
        "turn.html",
        chat=view,
...
async def stream_message(request: Request, chat_id: str, message_id: str) -> StreamingResponse:
    """SSE для HTMX: переводит события из chat.stream_answer в HTML-фрагменты."""
    chat = chat_service.get_chat(chat_id)
    if chat is None:
        raise HTTPException(status_code=404, detail="Чат не найден")
    assistant_msg = next((m for m in chat.messages if m.id == message_id), None)
    ...
    async def events() -> AsyncIterator[str]:
        # Стрим по этому сообщению уже запущен другим коннектом — не генерим второй раз.
        if message_id not in _streaming:
            ...
            return
        if lock.locked():
            yield chat_service.sse("token", render_markdown(assistant_msg.content))
            yield chat_service.sse("done", "")
            return

        try:
            async for chunk in chat_service.stream_answer(chat_id, assistant_msg):
                if chunk.startswith("event: tool_call"):
                    payload = json.loads(chunk.split("data: ", 1)[1].splitlines()[0])
                    yield chat_service.sse(
                        "tool", _tool_call_html(payload["name"], payload.get("kwargs") or {})
                    )
                    ...
                    yield chat_service.sse(
                        "tool",
                        _tool_result_html(
                            payload["name"], payload.get("output") or "", bool(payload.get("error"))
                        ),
                    )
                elif chunk.startswith("event: token"):
                    payload = json.loads(chunk.split("data: ", 1)[1].splitlines()[0])
                    yield chat_service.sse("token", render_markdown(payload.get("content", "")))
                elif chunk.startswith("event: error"):
                    payload = json.loads(chunk.split("data: ", 1)[1].splitlines()[0])
                    yield chat_service.sse("token", render_markdown(f"\n\n[ошибка] {payload['message']}"))
                elif chunk.startswith("event: redirect"):
                    # Заменяем всю панель: вместо формы появится плашка о переводе.
                    refreshed = chat_service.get_chat(chat_id)
                    view = ChatView(refreshed) if refreshed else None
                    html = templates.get_template("chat_panel.html").render(
                        chat=view,
                        chats=chat_service.list_chats(),
                        active_id=_active_id(),
                    )
                    yield chat_service.sse("panel", html)
